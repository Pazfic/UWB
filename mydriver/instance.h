...
*******************************************************************************************************************/
#define DEEP_SLEEP \
    (1) // To enable deep-sleep set this to 1
        // DEEP_SLEEP mode can be used, for example, by a Tag instance to put the DW1000 into low-power deep-sleep mode:
        //  when the Anchor is sending the range report back to the Tag, the Tag will enter sleep after a ranging
        //  exchange is finished once it receives a report or times out, before the next poll message is sent (before
        //  next ranging exchange is started).
...
#define ANCH_RESPONSE_MSG_LEN \
    11 // FunctionCode(1),Range Num (1), Sleep Correction Time (2), Measured_TOF_Time(4), aircmd (2),crc(1)
#define TAG_FINAL_MSG_LEN \
    42 // FunctionCode(1), Range Num (1), Poll_TxTime(5),
       // Resp0_RxTime(5), Resp1_RxTime(5), Resp2_RxTime(5), Resp3_RxTime(5),
       // Resp4_RxTime(5),Resp5_RxTime(5),Final_TxTime(5)
...
#define FRAME_CRTL_AND_ADDRESS_L \
    (FRAME_DEST_ADDRESS_L + FRAME_SOURCE_ADDRESS_L + FRAME_CTRLP) // 21 bytes for 64-bit addresses)
#define FRAME_CRTL_AND_ADDRESS_S \
    (FRAME_DEST_ADDRESS_S + FRAME_SOURCE_ADDRESS_S + FRAME_CTRLP) // 9 bytes for 16-bit addresses)
#define FRAME_CRTL_AND_ADDRESS_LS \
    (FRAME_DEST_ADDRESS_L + FRAME_SOURCE_ADDRESS_S + FRAME_CTRLP) // 15 bytes for 1 16-bit address and 1 64-bit address)
#define MAX_USER_PAYLOAD_STRING_LL \
    (STANDARD_FRAME_SIZE - FRAME_CRTL_AND_ADDRESS_L - TAG_FINAL_MSG_LEN - FRAME_CRC) // 127 - 21 - 16 - 2 = 88
#define MAX_USER_PAYLOAD_STRING_SS \
    (STANDARD_FRAME_SIZE - FRAME_CRTL_AND_ADDRESS_S - TAG_FINAL_MSG_LEN - FRAME_CRC) // 127 - 9 - 16 - 2 = 100
#define MAX_USER_PAYLOAD_STRING_LS \
    (STANDARD_FRAME_SIZE - FRAME_CRTL_AND_ADDRESS_LS - TAG_FINAL_MSG_LEN - FRAME_CRC) // 127 - 15 - 16 - 2 = 94

// NOTE: the user payload assumes that there are only 88 "free" bytes to be used for the user message (it does not scale
...
#define INST_DONE_WAIT_FOR_NEXT_EVENT \
    1 // this signifies that the current event has been processed and instance is ready for next one
#define INST_DONE_WAIT_FOR_NEXT_EVENT_TO \
    2 // this signifies that the current event has been processed and that instance is waiting for next one with a
      // timeout which will trigger if no event coming in specified time
...
#define VRESP \
    32 // Mask of valid response times (e.g. if bit 1 = A0's response time is valid)
       // FINAL message
...
typedef enum inst_states {
    TA_INIT, // 0 - Tag and Anchor Initialization

    // message
    TA_NULL,

    // middle state, doesn't change instance's previous state.
    TA_RXE_WAIT,
    TA_TXE_WAIT,     // 3 - state in which the instance will enter sleep (if ranging finished) or proceed to transmit a
    TA_TX_WAIT_CONF, // 4 - confirmation of TX done message

    TA_SLEEP,      // 5 - sleeping, waiting 4 waittie increasing to 0
    TA_SLEEP_DONE, // 6 - sleeping done

    TA_RX_WAIT_POLL,         // 7 - Anchor state, waiting 4 revieving POLL message
    TA_TXPOLL_WAIT_SEND,     // 8 - Tag state, configuration and send POLL message
    TA_RX_WAIT_CAND,         // 9 - Tag state, waiting 4 recieving CAND message
    TA_TXCAND_WAIT_SEND,     // 10 - Anchor state, configuration and send CAND message
    TA_RX_WAIT_GRANT,        // 11 - Anchhor state, waiting 4 recieving GRANT message
    TA_TXGRANT_WAIT_SEND,    // 12 - Tag state, configuration and send GRANT message
    TA_RX_WAIT_RESPONSE,     // 13 - Tag state, waiting 4 recieving RESPONSE message
    TA_TXRESPONSE_WAIT_SEND, // 14 - Anchor state, configuration and send RESPONSE message

    TA_FEEDBACK_DATA, // 15 - collect location data done, feedback to upper device
} INST_STATES;
...
typedef struct {
    ...
    uint8 fcs[2]; //  125-126  we allow space for the CRC as it is logically part of the message. However ScenSor TX
                  //  calculates and adds these bytes.
} srd_msg_dlsl;
