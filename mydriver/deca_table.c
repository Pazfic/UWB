...
int deca_configure(void)
{
	...
	if(DWT_DEVICE_ID != deID) //if the read of device ID fails, the DW1000 could be asleep
	{
			...
			if(DWT_DEVICE_ID != deID)
					return(-1) ;
			//clear the sleep bit - so that after the hard reset below the DW does not go into sleep
			//dwt_softreset();
	}
	//spi_set_rate_low();	
  result=dwt_initialise(DWT_LOADUCODE | DWT_READ_OTP_PID | DWT_READ_OTP_LID |DWT_READ_OTP_BAT | DWT_READ_OTP_TMP );//初始化DW1000
	dwt_setlnapamode(DWT_LNA_ENABLE | DWT_PA_ENABLE);//开功放
	...
	dwt_configuresleep(DWT_LOADUCODE|DWT_PRESRV_SLEEP|DWT_CONFIG|DWT_TANDV, DWT_WAKE_WK|DWT_WAKE_CS|DWT_SLP_EN);//配置睡眠
	dwt_setleds(3);//设置dw1000收发指示灯
	dwt_setinterrupt(DWT_INT_TFRS | DWT_INT_RFCG | DWT_INT_RFTO | DWT_INT_RXPTO | DWT_INT_RPHE | DWT_INT_RFCE | DWT_INT_RFSL | DWT_INT_SFDT, 1);//配置中断模式
...
//	power |= (power << 8) + (power << 16) + (power << 24);		
	...
	dwt_configuretxrf(&txconfig);//配置发射功率
  ...
  dwt_settxantennadelay(TX_ANT_DLY);		//设置发射天线延迟
	...
	dwt_setcallbacks(twr_tx_tag_cb, twr_rx_cb, twr_rx_timeout_cb, twr_rx_error_cb);//设置中断回调
	return(deID) ;
}	
