...
int readfromspi(uint16 headerLength,  uint8_t *headerBuffer, uint32 readlength, uint8 *readBuffer)
{
    int i;
    decaIrqStatus_t  stat ;
    stat = decamutexon() ;
...
/*    for(i=0; i<headerLength; i++)
    ...
    for(i=0; i<headerLength; i++)
    {
    	...
    	//HAL_SPI_TransmitReceive(&hspi1, &headerBuffer[i], (uint8 *)&temp, 1, HAL_MAX_DELAY);
    }
    HAL_SPI_TransmitReceive(&hspi1, &headerBuffer[i], (uint8 *)readBuffer, readlength, HAL_MAX_DELAY);
