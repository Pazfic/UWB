...
//	u8 i=0;
	...
	{	
		if((RXbuff[11]=='-')			
			&&(RXbuff[8]  <='9')&&(RXbuff[8]  >='0' )&&(RXbuff[9]  <='9')&&(RXbuff[9]  >='0')
		  &&(RXbuff[10] <='9')&&(RXbuff[10] >='0' )&&(RXbuff[12] <='9')&&(RXbuff[12] >='0')
		  &&(RXbuff[13] <='9')&&(RXbuff[13] >='0' )&&(RXbuff[14] <='9')&&(RXbuff[14] >='0')
  ...
  if(strstr((char *)RXbuff,"AT+TWLT=") != NULL)//0x06
	{
		if(RXbuff[8] == '1')
		{
			...
			{
				if((RXbuff[9+5*t1]==',')
					&&(((RXbuff[10+5*t1] <='9') && (RXbuff[10+5*t1] >='0'))||((RXbuff[10+5*t1] <='F')&&(RXbuff[10+5*t1] >='A')))
					&&(((RXbuff[11+5*t1] <='9') && (RXbuff[11+5*t1] >='0'))||((RXbuff[11+5*t1] <='F')&&(RXbuff[11+5*t1] >='A')))
					&&(((RXbuff[12+5*t1] <='9') && (RXbuff[12+5*t1] >='0'))||((RXbuff[12+5*t1] <='F')&&(RXbuff[12+5*t1] >='A')))
					&&(((RXbuff[13+5*t1] <='9') && (RXbuff[13+5*t1] >='0'))||((RXbuff[13+5*t1] <='F')&&(RXbuff[13+5*t1] >='A')))
					)
				x1++;
			}
			if(x1 != 5)
	...
	if(strstr((char *)RXbuff,"AT+BEEP?") != NULL)
			return BEEp;	
	if(strstr((char *)RXbuff,"AT+BEEP=") != NULL)
			return BEEP;	
	if(strstr((char *)RXbuff,"AT+UCHG=") != NULL)
			return UCHG;	
	if(strstr((char *)RXbuff,"AT+LIAP=") != NULL)
			return LIAP;
	if(strstr((char *)RXbuff,"AT+UCHG?") != NULL)
			return UCHg;		
	if(strstr((char *)RXbuff,"AT+BAUD?") != NULL)
			return BAUd;		
		...
		if(RXbuff[8] == 'F' /*|| RXbuff[8] == 'S' || RXbuff[8] == 'M'*/)
			if(RXbuff[9] == '2' || RXbuff[9] == '5')
				if(RXbuff[10] == '-')	
					...
					{
						if((((RXbuff[12] <='9') && (RXbuff[12] >='0'))||((RXbuff[12] <='F')&&(RXbuff[12] >='A')))
						 &&(((RXbuff[13] <='9') && (RXbuff[13] >='0'))||((RXbuff[13] <='F')&&(RXbuff[13] >='A')))
						 &&(((RXbuff[14] <='9') && (RXbuff[14] >='0'))||((RXbuff[14] <='F')&&(RXbuff[14] >='A')))
						 &&(((RXbuff[15] <='9') && (RXbuff[15] >='0'))||((RXbuff[15] <='F')&&(RXbuff[15] >='A')))
						)
...
//////////////////////////////////////////////////////////////////			
	if(cmd_value == GAIN)//AT+GAIN=xxxxxxxx
	{
		...
		n =	sprintf((char*)&cmd_buf[0],"OK+GAIN=0x%c%c%c%c%c%c%c%c\r\n",temp_rx_buf[8],temp_rx_buf[9]
			,temp_rx_buf[10],temp_rx_buf[11],temp_rx_buf[12]
			,temp_rx_buf[13],temp_rx_buf[14],temp_rx_buf[15]);
...
void Process_USART3_ATCMD(uint8 *temp_rx_buf,uint8 *temp_tx_buf)
{
	...
	if(USART3_RX_STA&0x8000)	   //判断是否接收完数据
	{	
...
void Process_USB_ATCMD(uint8 *temp_rx_buf,uint8 *temp_tx_buf)
{
	...
	if(USB_USART_RX_STA&0x8000)	   //判断是否接收完数据
	{	
		USB_USART_RX_STA = 0;
...
//	{
	...
	sys_para.TAG_SLOT=(RXbuff[12]-'0')*1000+(RXbuff[13]-'0')*100+(RXbuff[14]-'0')*10+(RXbuff[15]-'0');
...
{
/*	Process_USB_ATCMD(USB_USART_RX_BUF,USB_TxBuff);
//	
	if(tcp_client_flag&1<<6)
	{
		Process_LAN8742_ATCMD(tcp_client_recvbuf,tcp_client_sendbuf);
...
void display_deviceinfo(System_Para_TypeDef *syspara, uint16_t type)
{
  ...
  unsigned char cRevisionDate[12] = __DATE__;
	...
	if(( type&INFO_VERSION) == INFO_VERSION)
	{
		...
		//n+=	sprintf((char*)&Sendbuffusb[n],   ">> MCU UniqueID: %08x-%08x-%08x\r\n",*(uint32_t*)(0x1FFF7A10),*(uint32_t*)(0x1FFF7A14),*(uint32_t*)(0x1FFF7A18) );
		//n+=	sprintf((char*)&Sendbuffusb[n],   ">> Start Number: %d\r\n",syspara->start_count );
