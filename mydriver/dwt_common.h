...
extern int   MAX_TAG_SIZE ;		//标签个数
extern int   N_Slot       ;	  //时间片个数
extern int   T_Slot       ;		//1个时间片的时间 ms
extern int   T_Round  		;//轮询的总时间
/**********************************************************************************************/
...
//typedef struct
//{
//	uint16 POLL_TX_TO_RESP_RX_DLY_UUS;//poll包发送后延迟接收response包时间
//	
//	uint16  RESP_RX_TO_FINAL_TX_DLY_UUS ;//final包发送时间参数

//	uint16  RESP_RX_TIMEOUT_UUS ;//respon包接收超时时间

//	uint16 POLL_RX_TO_RESP_TX_DLY_UUS ;//response包发送时间参数

// 	uint16 RESP_TX_TO_FINAL_RX_DLY_UUS ;//response包发送后延迟接收final包时间

// 	uint16 FINAL_RX_TIMEOUT_UUS ;//final包接收超时时间
